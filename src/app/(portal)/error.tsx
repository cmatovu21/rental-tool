...
export default function PortalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="bg-white border border-rule rounded-lg p-8 text-center max-w-md mx-auto mt-12">
      ...
      <p className="text-sm text-ink/60 mb-6">This page hit an unexpected error. Try again, or head back home.</p>
      <div className="flex gap-3 justify-center">
        <Button type="button" fullWidth={false} className="px-5" onClick={reset}>
          Try again
        </Button>
        <a href="/portal" className="text-sm text-forest hover:underline self-center">
          Back home
        </a>
      </div>
    </div>
